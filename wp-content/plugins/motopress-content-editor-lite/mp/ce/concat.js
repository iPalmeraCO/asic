steal('jquery/class', function($) {
    $.Class('MP.Editor',
    {
        ...
        on: function(eventName, callback){
            eventName = eventName.split(' ').map(function(eName){return MP.Editor.eventPrefix + eName}).join(' ');
...
steal('jquery/controller', function($) {
    $.Controller('MP.Flash',
    {
        ...
        setFlash: function(message, type) {
            ...
            if (cssClass.length) alert.addClass(cssClass);
            if (message.length) alert.children('span').html(message);
        },
    ...
    },
    {
        ...
        create: function() {
            return this.alertEl
                .clone()
                .append(this.closeEl.clone(), this.messageEl.clone())
...
steal('jquery/class', function($) {
    $.Class('MP.Utils',
    {
        ...
        utf8_decode: function(str_data) {
            ...
            while(i<str_data.length){c1=str_data.charCodeAt(i);if(c1<128){tmp_arr[ac++]=String.fromCharCode(c1);i++;}else if(c1>191&&c1<224){c2=str_data.charCodeAt(i+1);tmp_arr[ac++]=String.fromCharCode(((c1&31)<<6)|(c2&63));i+=2;}else{c2=str_data.charCodeAt(i+1);c3=str_data.charCodeAt(i+2);tmp_arr[ac++]=String.fromCharCode(((c1&15)<<12)|((c2&63)<<6)|(c3&63));i+=3;}}
            return tmp_arr.join('');
        },
...
steal('jquery/controller', function($){
    ...
    },
    {
        ...
        divDisablePlugin: $('<div />', {
            'class': 'motopress-disable-plugin'
        }),
        ...
        onLoad: function() {
	        ...
	        this.body.find('.motopress-content-wrapper:not(:first)').remove();
            ...
            var stealVerScript = $('<script />', {
                text: 'var steal = { production: "mp/ce/iframeProd/production" + parent.motopress.scriptSuffix + ".js" + parent.motopress.pluginVersionParam };'
            })[0];
            this.appendScript(head, stealVerScript);

            var script = $('<script />', {
                src: MP.Settings.pluginDirUrl + 'steal/steal.production.js?mp/ce/iframeProd'
            })[0];
	        ...
	        this.sceneContent
                .wrap('<div class="'+CE.Iframe.myThis.gridObj.span.fullclass+'" />').parent()
                .wrap('<div class="'+CE.Iframe.myThis.gridObj.row.class+' '+CE.Iframe.myThis.gridObj.row.edgeclass+' motopress-row" />').parent()
                .wrap('<div id="motopress-container" class="mp-container" />');
            ...
            $('body').hide(0, function() { $('body').show(); });
        },
	    ...
	    setSceneWidth: function(sceneContentWidth) {
		    ...
		    var docWidth = CE.Iframe.contents.width(),
	            rowExampleMarginLeft = parseFloat(this.rowExample.css('margin-left')),
	            rowExampleMarginRight = parseFloat(this.rowExample.css('margin-right'));

		    sceneContentWidth = sceneContentWidth - rowExampleMarginLeft - rowExampleMarginRight;

	        if (sceneContentWidth > docWidth) sceneContentWidth = docWidth + (rowExampleMarginLeft + rowExampleMarginRight);
        ...
        unwrapGrid: function() {
            this.body.find('div[data-motopress-group="mp_grid"]').each(function() {
                ...
                if (attrs.length) child.attr('data-motopress-parameters', attrs);
                if (styles.length) child.attr('data-motopress-styles', styles);
                child.unwrap();
            });
        },
        ...
        renderContent: function(title, editableContent, pageTemplate) {
            ...
            if (typeof editableContent === 'undefined') editableContent = '';
            if (typeof pageTemplate === 'undefined') pageTemplate = '';
	        ...
	        if (postStatus === 'auto-draft') {
		        var oldTarget = this.$postForm.attr('target'),
			        sceneFormFields = this.$form.find('.mpce-form-fields:eq(0)');
		        ...
		        this.$postFormMPFields.find('[name]').each(function() {
			        $(this).val(
				        sceneFormFields.find('[name="' + $(this).attr('name') + '"]').val()
			        );
		        });
        ...
        getTinyMCEContent: function() {
	        ...
	        var self = this,
		        ...
		        pageTemplateEl = $('[name="page_template"]'),
		        pageTemplate = pageTemplateEl.length ? pageTemplateEl.val() : '';
	    ...
	    preventUnload: function() {
		    this.body.find('a').on('click', function(e) { e.preventDefault(); });
	        this.body.find('form').on('submit', function(e) { e.preventDefault(); });

		    var isParentReload = false;
		    $(window).on('beforeunload.CE.Iframe', function() { isParentReload = true; });
	        $(CE.Iframe.window).on('beforeunload.CE.Iframe', function() {
		        if (!isParentReload) return localStorage.getItem('postSaveAlert');
...
steal('jquery/controller', function($) {
    ...
    },
    {
        ...
        init: function(el) {
	        ...
	        $('#motopress-ce-btn').on('click', function() {
            ...
            if (this.editorWrapperEl.height() === 0) {
                ...
                if (h1 > 0) this.editorWrapperEl.height(h1);
                var h2 = this.editorEl.parent().height() - 32;
                if (h2 > 0) this.editorEl.height(h2);
                $(window).resize(function() {
                    var h1 = CE.Navbar.myThis.editorWrapperEl.parent().height() - 53;
                    if (h1 > 0) CE.Navbar.myThis.editorWrapperEl.height(h1);
                    var h2 = CE.Navbar.myThis.editorEl.parent().height() - 32;
                    if (h2 > 0) CE.Navbar.myThis.editorEl.height(h2);
                });
            }
        ...
        '#motopress-content-editor-tutorials click': function(e) {
            var modalData = jQuery("#motopress-tutorials-modal"),
                bodyBG = jQuery("body");
	    ...
	    renderTitle: function(value) {
	        ...
	        this.$titles.each(function() {
		        $(this).html(
			        $(this).html().replace(
				        new RegExp(self.hiddenCharUnicode + '.*' + self.hiddenCharUnicode, 'g'),
				        self.hiddenChar + value + self.hiddenChar
			        )
		        );
	        });
	    },
        ...
        showContentEditor: function() {            
            ...
            $('#wpwrap')
                .height('100%')
                .children('#wpcontent').children('#wpadminbar').prependTo('body');
            this.editorEl.show();
        },
...
steal('jquery/class', function($) {
    $.Class("CE.Save", {
            ...
            dom.children('.motopress-row, .mpce-wp-more-tag').each(function() {
                ...
                if (level === 1 && row.hasClass('mpce-wp-more-tag')) {
                    ...
                    rowEdge.children('.motopress-clmn').each(function() {
                        var span = $(this),
		                    spanEdge = MP.Utils.getEdgeSpan(span),
	                        spanClass = MP.Utils.getSpanClass(span.prop('class').split(' ')),
	                        col = MP.Utils.getSpanNumber(spanClass),
	                        style = '',
	                        mpSpanAttr = '';
        ...
        getAttributes: function(obj){
            ...
            var result = '';
            if (attributes) {
                var shortcodeGroup = obj.attr('data-motopress-group'),
                shortcodeName = obj.attr('data-motopress-shortcode');
                ...
                $.each(attributes, function(key, attrs) {
                    if (shortcodeParameters.hasOwnProperty(key)
                        && shortcodeParameters[key].hasOwnProperty('saveInContent')
                        && shortcodeParameters[key].saveInContent == 'true') {
                            attrs.value = '';
                    }

                    if ( $.inArray(shortcodeName, ['mp_row', 'mp_row_inner']) !== -1){

                        if (shortcodeParameters.hasOwnProperty(key)
                            && typeof(shortcodeParameters[key].disabled) !== 'undefined'
                            && shortcodeParameters[key].disabled === 'true'
                            && $.inArray(key, ['bg_video_youtube', 'bg_video_youtube_cover', 'bg_video_webm', 'bg_video_mp4', 'bg_video_ogg', 'bg_video_cover', 'parallax_image']) !== -1) {
                            return;
                        }
                        ...
                        if ( shortcodeParameters.hasOwnProperty(key)
                            && shortcodeParameters[key].dependency
                            && attributes[shortcodeParameters[key].dependency.parameter].value !== shortcodeParameters[key].dependency.value
                            && (typeof(shortcodeParameters[key].disabled) === 'undefined' || shortcodeParameters[key].disabled === 'false') ) {
                            return;
                        }
                    }
                    if (typeof attrs.value !== 'undefined' && attrs.value !== '') result += ' ' + key + '="' + attrs.value + '"';
                });
            }
            return result;
        },
...
steal('jquery/class', function($) {
    ...
    {
        setup: function() {
            if (typeof CE === 'undefined') this.removeWpPanels();
...
steal('jquery/controller', function($) {
    $.Controller('CE.CodeModal',
    {
        myThis: null,
        currentShortcode: null,
        currentTextareaTinymce: null,
        listensTo: ['show', 'shown', 'hide']        
    },
    {
        iframeBody: null,
        content: null,
        ...
        setSize: function(isHidden) {
            ...
            var containerHeight = this.element.find('#wp-motopresscodecontent-wrap').height() - this.element.find('#wp-motopresscodecontent-editor-tools').outerHeight()
                - parseFloat(this.container.css('border-top-width')) - parseFloat(this.container.css('border-bottom-width')) - magicPixels;
        ...
        recalcTabSizeWPOld: function(){
            var contentHeight = this.container.height() - parseFloat(this.container.css('border-top-width')) - parseFloat(this.container.css('border-bottom-width'))
                - this.element.find('#qt_motopresscodecontent_toolbar').outerHeight()
                - parseFloat(this.content.css('padding-top')) - parseFloat(this.content.css('padding-bottom'));
            ...
            var contentIfrHeight = this.container.height() - parseFloat(this.container.css('border-top-width')) - parseFloat(this.container.css('border-bottom-width'))
                - mceFirst.outerHeight() - mceLast.outerHeight();
        ...
        show: function(el, e) {
            ...
            if (
                CE.Iframe.window.CE.Dialog.myThis.element.dialog('isOpen') &&
                !el.data('modal').hasOwnProperty('closeDialog')
            ) {
        ...
        hide: function(el, e) {
            if (!this.isHiding){ 
                this.isHiding = true;
                var handle = CE.CodeModal.currentShortcode
                    .closest('.motopress-clmn')
                    .find('.' + CE.Iframe.window.CE.DragDrop.myThis.helper.attr('class'))
                    .first()
                    .children('.' + CE.Iframe.window.CE.DragDrop.myThis.dragHandle.attr('class'));
                handle.prev().focus();
                this.content.val('');
                if (this.editor !== null) this.editor.setContent('');
                this.saveHandler = null;
                if (CE.Iframe.wpEditor !== null) CE.Iframe.wpEditor.focus();
