...
steal('jquery/class', function($) {
    ...
    },
    {
	    ...
	    initEvents: function() {
		    this.$contentWrapper
			    .on('click', '>'+this.pointClass+', >.motopress-handle-middle-in>'+this.pointClass+'', this.proxy('onPointClick'))
		        .on('click', '>'+this.tagClass, this.proxy('onTagClick'));
	    ...
	    isPointExists: function(type) {
			return !!(
				...
				&& type
				&& this.contentSectionExists
				&& this.$contentWrapper.find(this.pointPosClassPrefix + type).length
			);
	    },
    ...
    },
    {
	    ...
	    setup: function(el) {
	        ...
	        for (var i = 1; i <= this.columnCount; i++) {
		        col = this.columnEl.clone().addClass(
                    parent.CE.Iframe.myThis.gridObj.span.class + i
		        ).appendTo(this.rowEl);

		        this.colWidthByNumber[i] = parseFloat(col.width());
            }

		    this.columnEl.clone().addClass(
                parent.CE.Iframe.myThis.gridObj.span.class + 1
	        ).appendTo(this.rowEl);
        },
...
steal( 'jquery/class', function($) {
    ...
    },
    {
        ...
        leftBarOverlap: $('<div />', {
            'class': 'motopress-leftbar-overlap'
        }),
	    widgetsWrapper: $('<div id="mpce-leftbar-widgets-wrapper" />'),
	    customWrapper: $('<div id="mpce-leftbar-custom-wrapper" />'),
        group: $('<div />', {
            'class': 'motopress-leftbar-group motopress-default'
        }),
        groupInner: $('<div />', {
            'class': 'motopress-leftbar-group-inner motopress-default'
        }),
        groupIcon: $('<div />', {
            'class': 'motopress-leftbar-group-icon'
        }),
        groupActive: $('<div />', {
            'class': 'motopress-leftbar-group-active motopress-default'
        }),
        object: $('<section />', {
            'class': 'motopress-ce-object motopress-default-drag'
        }),
        objectInner: $('<div />', {
            'class': 'motopress-ce-object-inner motopress-default'
        }),
        objectDot: $('<div />', {
            'class': 'motopress-ce-object-dot motopress-default'
        }),
        objectIcon: $('<div />', {
            'class': 'motopress-ce-object-icon'
        }),
        objectName: $('<span />', {
            'class': 'motopress-ce-object-name motopress-default motopress-no-color-text'
        }),
        ...
        setup: function() {            
            ...
            this.groupIcon
                .append(this.groupActive)
                .appendTo(this.groupInner);

            this.groupInner
                .append(this.groupPopover)
                .appendTo(this.group);

            this.objectInner
                .append(this.objectDot, this.objectIcon, this.objectName)
        ...
        init: function() {
            ...
            if (templatesEl.length) new CE.Template(templatesEl);
	    ...
	    initCustom: function() {
		    ...
		    groupStr =
			    '<div class="mpce-leftbar-custom-group-1">' +
					'<div class="motopress-leftbar-group motopress-default mpce-leftbar-more-btn" title="'+localStorage.getItem('CELeftBarWPMoreTitle')+'">' +
			            '<div class="motopress-leftbar-group-inner motopress-default">' +
			                '<div class="motopress-leftbar-group-icon mpce-leftbar-more-icon">' +
								'<div class="motopress-leftbar-group-active motopress-default"></div>' +
							'</div>' +
				        '</div>' +
				    '</div>';
		    if (!parent.motopressCE.settings.wp.white_label_active) {
			    groupStr +=
				    '<div class="motopress-leftbar-group motopress-default mpce-leftbar-addons-btn" title="'+localStorage.getItem('CELeftBarAddonsTitle')+'">' +
			            '<div class="motopress-leftbar-group-inner motopress-default">' +
			                '<div class="motopress-leftbar-group-icon mpce-leftbar-addons-icon">' +
								'<a href="'+parent.motopressCE.settings.wp.home_addons_url+'" target="_blank" class="motopress-leftbar-group-active motopress-default"></a>' +
							'</div>' +
				        '</div>' +
				    '</div>';
		    }
		    ...
		    $.each(['more', 'addons'], function(key, name) {
			    $btns[name] = $group.find('.mpce-leftbar-'+name+'-btn')
		    });

            $group.find('.motopress-leftbar-group')
	            .on('mouseover', function() {
	                self.hideActiveWidgetsGroup();
	            })
	            .on('mouseout', function() {
	                $(this).find('.motopress-leftbar-group-active').removeClass('active');
	            });

            $btns['more'].find('.motopress-leftbar-group-active').on('mousedown', function() {
        ...
        initLibraryStyles: function() {
            var library = parent.motopressCE.settings.library,
	            limitations, disableAllPaddings, disableAllMargins, disableAllBorderWidth;
                ...
                for (var g in this.library) {
                    if (this.library[g]['show']) {
                        ...
                        group.clickover({
                            ...
                            onShown: function() {
                                this.$tip.find('.arrow').removeClass('arrow').addClass('motopress-ce-arrow');

                                var shadow = this.$element.is(':first-child') ? 5 : 0;
                                var tipTop =  parseInt(this.$tip.css('top')) + (parseInt(this.$tip.css('height')) / 2) - 23 + shadow;
                                this.$tip.css('top', tipTop);

                                var tipTitle = this.$tip.find('.popover-title');
                            ...
                            onHidden: function() {
                                this.$element.find('.motopress-leftbar-group-active').removeClass('active');
                            }
                        });
	                    group.find('.motopress-leftbar-group-active').on('mousedown', function(e) {
	                        e.preventDefault();
	                        e.stopPropagation();
                        });
        ...
        hoverActive: function() {
            $('.motopress-leftbar-group .motopress-leftbar-group-active').mouseover(function() {
	            $(this).addClass('active');
            });
        },

        disable: function() { this.leftBarOverlap.show(); },
        enable: function() { this.leftBarOverlap.hide(); },

        setActive: function() { this.active = true; },
        setInactive: function() { this.active = false; },
        isActive: function() { return this.active; },

	    setVisible: function() { this.visibility = true; },
	    setInvisible: function() { this.visibility = false; },
	    isVisible: function() { return this.visibility; },
    ...
    },
    {
        ...
        init: function() {
            ...
            this.frame = parent.wp.media({
                ...
                button: {
                    text: localStorage.getItem('CEImageLibraryText')
                },
                library: {
                    type: 'image'
                },
        ...
        onOpen: function() {
            ...
            var imgCtrl = this.propertyImage.controller();
            if (imgCtrl.id !== null) {
...
steal('jquery/class', function($) {
    ...
    },
    {
        ...
        open: function(element, showObj, submitCallback, cancelCallback) {
            var show = {url: true, title: true, target: true};
            ...
            media.controller.CEGallery = media.controller.FeaturedImage.extend({
                ...
                updateSelection: function () {
                    var selection = this.get('selection'),
                        ids = CE.WPGallery.myThis.ctrl.get(),
                        attachments;
                    if ('' !== ids && -1 !== ids) {
...
steal('jquery/class', function($) {
    ...
    },
    {
        ...
        oldShortcode: null,
        dialog: null,
        minimizeRestoreBtn: $('<button />', {
            'class': 'ui-dialog-titlebar-minimize'
        }),
        state: {
            minimized: 'minimized',
            restored: 'restored'
        ...
        tabs: $('<div />', {
            id: 'motopress-dialog-tabs'
        }),
        settingsTitle: $('<li />'),
        styleTitle: $('<li />'),
        settingsTab: $('<div />', {
            id: 'motopress-dialog-settings-tab'
        }),
        styleTab: $('<div />', {
            id: 'motopress-dialog-style-tab'
        }),
        ...
        setup: function() {
            ...
            $('<a />', {
                ...
                'class': 'motopress-settings-icon'
            })).appendTo(this.settingsTitle);
            $('<a />', {
                href: '#' + this.styleTab.attr('id'),
                'class': 'motopress-text-no-color-text',
                text: localStorage.getItem('CEStyle')
            }).append($('<i />', {
                'class': 'motopress-style-icon'
            })).appendTo(this.styleTitle);
            ul.append(this.settingsTitle, this.styleTitle);
            this.tabs
                .append(ul, this.settingsTab, this.styleTab)
        ...
        updateTitle: function(){
            var shortcodeCtrl = this.shortcode.controller();
            switch(this.getMode()) {
                case 'tabs':
                    this.setTitle(shortcodeCtrl.shortcodeLabel);
                    break;
                case 'style-editor':
                    var styleEditorCtrl = this.styleEditorContainer.children().controller();
        ...
        init: function() {
            ...
            var position = (left !== null && top !== null) ? [parseInt(left), parseInt(top)] : {my: 'right-20 top+20', at: 'right top', of: window};
	        ...
	        this.element.dialog({
                    ...
                    create: function() {
                        ...
                        dialog.on('resizestart', function(e, ui) {
                            ...
                            if (jqVersion) {
                                if (!isHandleN) $(this).css('top', parseInt($(this).css('top')) - scrollTop);
                            }
                        });
                        dialog.on('resize', function(e, ui) {
                            if (jqVersion && isHandleN) $(this).css('top', parseInt($(this).css('top')) - scrollTop);
                        });
                    ...
                    open: function() {
                        ...
                        if (selected.length) CE.Selectable.focusWithoutScroll(selected.prev());
                        ...
                        if (dTop < 0) dialog.css('top', 0);
                        else if (dHeight < wHeight && dTop + dHeight > wHeight) dialog.css('top', wHeight - dHeight);
                        else if (dTop + dHeight > wHeight) dialog.css('top', wHeight - dHeight);
                        else dialog.css('top', dTop);
                        if (dLeft < 0) dialog.css('left', 0);
                        else if (dWidth < wWidth && dLeft + dWidth > wWidth) dialog.css('left', wWidth - dWidth);
                        else dialog.css('left', dLeft);
                    },
        ...
        open: function(dragHandle) {
            ...
            if (
                    (typeof this.shortcode !== 'undefined' && this.shortcode && this.shortcode.length) &&
                    (typeof this.oldShortcode !== 'undefined' && this.oldShortcode && this.oldShortcode.length)
            ) {
            ...
            var ctrl = this.shortcode.controller(CE.Controls);
            ...
            if (CE.LeftBar.myThis.library !== null && typeof ctrl.group !== 'undefined' && typeof ctrl.shortcodeName !== 'undefined') {
                if (CE.LeftBar.myThis.library[ctrl.group].objects.hasOwnProperty(ctrl.shortcodeName)) {
                    ...
                    if (!isEmptyParameters || !$.isEmptyObject(CE.Style[styleProps])) {
                        ...
                        if (typeof shortcodeTab !== 'undefined') activeTab = shortcodeTab;
                        if (!isEmptyParameters) {
                            if (this.tabs.tabs('option', 'disabled') !== false) this.tabs.tabs('enable', this.settingsTitle.index());
                        } else {
        ...
        close: function() {
            if (this.getMode() === 'style-editor') {
                var formStyleEditorCtrl = this.styleEditorContainer.children().controller();
    ...
    },
    {
        ...
        init: function() {            
            ...
            this.frame = parent.wp.media({
                ...
                button: {
                    text: localStorage.getItem('CEMediaLibraryText')
                },       
                contentUserSetting: true,
                syncSelection: true
            });
        ...
        setMedia: function() {
            var attributes = this.frame.state().get('selection').models[0].attributes,
            	mediaCtrl = this.propertyMedia.controller();
    ...
    },
    {
        ...
        init: function() {
            ...
            this.frame = parent.wp.media({
                ...
                button: {
                    text: localStorage.getItem('CEVideoLibraryText')
                },
                library: {
                    type: 'video'
                },
        ...
        setVideo: function() {
            ...
            var url = attributes.url;
            var videoCtrl = this.propertyVideo.controller();
...
steal('jquery/controller', function($) {
    $.Controller("CE.Ctrl", {
        ...
        init: function(el, args) {            
            ...
            this.shortcodeCtrl = args.shortcode.controller();
        },
        ...
        hideDependencedControls : function(){
            ...
            $this.formCtrl.form.find('> [data-motopress-parameter] > .motopress-controls:not(".select2-container"), > [data-motopress-parameter] > :not(".motopress-property-legend, .motopress-property-label, .motopress-property-description, hr") > .motopress-controls').each(function() {
                dependencedCtrl = $(this).controller();
    ...
    CE.Ctrl("CE.CtrlSpinner", {
        ...
        init: function(el, args) {
            this._super(el, args);
            if (args.hasOwnProperty('min')) this.min = args.min;
            if (args.hasOwnProperty('max')) this.max = args.max;
            if (args.hasOwnProperty('step')) this.step = args.step;
        },

        'customize': function(el, e) {
            if (this.customized) return false;
            else this.customized = true;
        ...
        'change': function(el, e, oldValue) {
            ...
            if (value !== null && oldValue !== value && value >= this.min && value <= this.max && (value / this.step).toFixed(12) % 1 === 0) {
                CE.Ctrl.prototype.change.apply(this, [el, e]);
            }
        },
        ...
        '.motopress-property-button-default click': function(el) {
            ...
            CE.Link.myThis.open(input, {title: false, target: false}, function(atts) {
                if (input.val == atts.href) return;
                input.val(atts.href);
    ...
    CE.Ctrl("CE.CtrlTextareaTinymce", {}, {
        ...
        saveHandler: function(e) {
            ...
            $this.switchVisual();
            var controller = parent.CE.CodeModal.currentTextareaTinymce.controller();
    ...
    CE.Ctrl("CE.CtrlMedia", {
        listensTo: ['dialogOpen']
    }, {                
        init: function(el, args) {
            this._super(el, args);
    ...
    CE.Ctrl("CE.CtrlImageGeneral", {
        ...
        init: function(el, args) {
            this._super(el, args);
            if (args.isNew && args.autoOpen === 'true') this.autoOpen = true;
        }
    });

    CE.CtrlImageGeneral("CE.CtrlImage", {
        thumbnail: parent.MP.Settings.pluginDirUrl + 'images/ce/imageThumbnail.png' + parent.motopress.pluginVersionParam
    }, {
        returnMode: 'id',
        pseudoRender: false,
        value: null,        
        ...
        showPreloader: function() {
            this.element
                .children('.motopress-thumbnail-crop')
                    .addClass(CE.Shortcode.preloaderClass)
                    .children('.motopress-thumbnail')
        ...
        'change': function(el, e) {
            if (el.is(':checked')) {
                el.closest('.motopress-property-group-accordion-item').siblings().find('.motopress-property-group-accordion-item-content > [data-motopress-parameter="' + this.name + '"] > .motopress-controls').each(function() {
                    ...
                    if (ctrl.get() === 'true') {
                        ctrl.set('false');
                        CE.Ctrl.prototype.change.apply(ctrl, [$(this), e]);
                    }
                });
            }
            CE.Ctrl.prototype.change.apply(this, [el, e]);
    ...
    CE.Ctrl("CE.CtrlSelect", {
        ...
        'customize': function(el, args) {
                ...
                createSearchChoice: function(term, data) {
                    ...
                    if (allowedSymbolsRegExp.test(term)){
                        if ($(data).filter( function() {
                            return !this.hasOwnProperty('id') ? $(this.children).filter(function(){ return this.text.toUpperCase().localeCompare(term.toUpperCase())===0 || this.id.toUpperCase().localeCompare(term.toUpperCase())===0;}).length!==0 : this.text.toUpperCase().localeCompare(term.toUpperCase())===0 || this.id.toUpperCase().localeCompare(term.toUpperCase())===0;
                        }).length===0) {
                            return {id:term.toLowerCase(), text:term, custom: true};
                        }
                    }
                },
            ...
            el.on('select2-selecting', function(e){
                ...
                if (e.object.hasOwnProperty('custom') && e.object.custom === true){
                    self.data.push({'id':e.object.id, 'text': e.object.text, 'custom': e.object.custom});
                }
            ...
            selectedClasses.forEach( function(el, index){
                if ($(classes).filter( function(){
                    ...
                    if (this.hasOwnProperty('children')) {
                        res = $(this.children).filter( function() { return this.hasOwnProperty('id') ?  this.id.localeCompare(el)===0 : false;}).length!==0;
                    } else {
                        res = this.hasOwnProperty('id') ? this.id.localeCompare(el)===0 : false;
                    }
                    return res;
                    }).length===0){
                    classes.push({id: el, text: el});
                }
            });
        ...
        'getBasicClasses' : function(){
            ...
            if (!$.isEmptyObject(this.options.style.basic)) {
                if (this.options.style.basic.hasOwnProperty('class')){
                    basicClasses.push({id: this.options.style.basic['class'], text: this.options.style.basic.label, locked: true});
                } else {
                    $.each(this.options.style.basic, function(key, value){
                        basicClasses.push({id: value['class'], text: value.label, locked: true});
                    });
                }
                ...
                for (var group in data) {
                    if (data[group].hasOwnProperty('values')) {
                        ...
                        for (var el in data[group]['values']) {
                            children.push({id: data[group]['values'][el]['class'], text: data[group]['values'][el].label, 'disabled': data[group]['values'][el].hasOwnProperty('disabled') && data[group]['values'][el]['disabled'], 'external' : data[group]['values'][el]['external']});
                        }
                        result.push({text: data[group].label, children: children, allowMultiple: data[group].allowMultiple});
                    } else {
                        result.push({id: data[group]['class'], text: data[group]['label'], 'disabled': data[group].hasOwnProperty('disabled') && data[group]['disabled'], 'external' : data[group]['external']});
                    }
                }
            }
            return result;
            ...
            $.each(CE.Style.globalPredefinedClasses, function(key, value) {
                if (value.hasOwnProperty('values')) {
                    ...
                    $.each(value.values, function(k, val) {
                        children.push({id: val['class'], text: val.label, 'disabled': val.hasOwnProperty('disabled') && val['disabled']});
                    });
                    result.push({text: value.label, children: children, allowMultiple: value.allowMultiple});
                } else {
                    result.push({id: value['class'], text: value.label, 'disabled': value.hasOwnProperty('disabled') && value['disabled']});
                }
            });
            return result;
        }
        ...
        selectPreset: function(preset){
            ...
            this.element.select2('data', [this.getPrivateBtn(), {id: presetObj.getClassName(), text: presetObj.getLabel()}]);
        ...
        'customize': function(){
            if (this.customized) {                
                ...
                this.customized = true;
                        ...
                        this.privateStyleForm = $('<div />', {
                'class': 'motopress-style-editor-forms motopress-style-editor-private-forms'
            });
            this.presetStyleForm = $('<div />', {
                'class': 'motopress-style-editor-forms motopress-style-editor-preset-forms'
            });
            this.privateStyleFormCtrl = new CE.PrivateStyleControlsForm(this.privateStyleForm, {'formsMainCtrl': this.formCtrl.formsMainCtrl, 'styleEditorCtrl': this, 'editedStyleObj': this.privateStyleObj});
            this.presetStyleFormCtrl = new CE.PresetStyleControlsForm(this.presetStyleForm, {'formsMainCtrl': this.formCtrl.formsMainCtrl, 'styleEditorCtrl': this});
        ...
        getPrivateBtn: function(){
            return {id: this.privateStyleObj.getClassName(), text: 'Edit Element Style', locked: true};
        },
        ...
        'valueToData' : function(value){
            ...
            $.each(value, function(index, val){
                if (CE.StyleEditor.myThis.isExistsPreset(val)) {
                    ...
                    data.push({id: presetCtrl.getClassName(), text: presetCtrl.getLabel()});
        ...
        getUnsetClasses: function(clsName){
            ...
            return $.map(presets, function(element,index) {return index});
        },
        ...
        setStatePreview: function(state){
            ...
            var previewStateClasses = previewStates.map(function(val){return CE.ShortcodeStyle.previewStateClassPrefix + val;}).join(' ');            
        ...
        unsetStatePreview: function(){
            var allPreviewStateClasses = ['up', 'hover', 'mobile', 'tablet'].map(function(val){return CE.ShortcodeStyle.previewStateClassPrefix + val;}).join(' ');
            var targetElement = this.formCtrl.getTargetElement(this);
            if (targetElement !== null) {
    ...
    CE.Ctrl("CE.CtrlComplex", {
        ...
        init: function(el, args){
            ...
            this.innerFormCtrl = new CE.ControlsSubForm(this.innerForm, {'formsMainCtrl': this.formCtrl.formsMainCtrl, 'parentPropertyCtrl': this});
    ...
    CE.Ctrl("CE.CtrlColorPicker", {
        listensTo: ['customize', 'dialogOpen'],
        'paletteAddBtn' : $('<span />', {
            'class': 'palette-add'
        })
    }, {
        ...
        initPaletteAddBtn: function(){
            var self = this,
                paletteAddBtn = CE.CtrlColorPicker.paletteAddBtn.clone();
    ...
    CE.Ctrl("CE.CtrlGroup", {
        ...
        'render': function(el, e, isNew) {
                            ...
                            if (oldIndex >= 0 && $this.events.hasOwnProperty('onInactive')) {
                                if (children.eq(0).closest('[data-motopress-shortcode="' + childName + '"]').length) {
                                    activeChild = children
                                        .closest('[data-motopress-shortcode="' + childName + '"]')
                                        .eq(oldIndex)
                                        .find($this.rules.rootSelector);
                                } else {
                                ...
                                if (activeChild.length && (!$this.hasActiveRules || ($this.hasActiveRules && activeTagItem.hasClass($this.rules.activeClass)))) {
                                    if ($this.events.onInactive.selector) {
                                        activeChild
                                            .find($this.events.onInactive.selector)
                                            .triggerHandler($this.events.onInactive.event);
                                    } else {
                            ...
                            if (newIndex >= 0 && $this.events.hasOwnProperty('onActive')) {
                                ...
                                if (activeChild.length && (!$this.hasActiveRules || ($this.hasActiveRules && !activeTagItem.hasClass($this.rules.activeClass)))) {
                                    if ($this.events.onActive.selector) {
                                        activeChild
                                            .find($this.events.onActive.selector)
                                            .triggerHandler($this.events.onActive.event);
                                    } else {
                                    ...
                                    if ($this.hasActiveRules) {
                                        if ($this.rules.activeSelector) {
                                            children
                                                .find($this.rules.activeSelector)
                                                .removeClass($this.rules.activeClass);
                                        } else {
            ...
            if (this.disabled) {
                if (
                    !isNew ||
                    isNew && this.accordion.children('.motopress-property-group-accordion-item').length
                ) {
        ...
        '> .motopress-property-button-wrapper > .motopress-property-button-default click': function(el, e, count) {
            e.stopPropagation();
                ...
                if (attrs !== null) CE.LeftBar.myThis.setAttrs(newItem, this.shortcodeGroup, attrs);
        ...
        updateLabel: function() {
            if (this.label.hasOwnProperty('parameter')) {
                ...
                this.accordion.find('.motopress-property-group-accordion-item-label-text').each(function() {
                    var formCtrl = $(this).closest('.motopress-property-group-accordion-item').controller();
    ...
    CE.Ctrl("CE.CtrlMargin", {}, {
        ...
        get: function() {
            ...
            $.each($this.margin.sides, function(i, side) {
                ...
                margin.push(val);
                if (val !== $this.margin.values[0]) allNone = false;
            });
...
steal('jquery/class', function($) {
    $.Class("CE.CtrlTemplates",
    {
        ...
        createInput: function(attrs, name, type, className) {
            ...
            if (typeof className === 'undefined') className = 'input';
            var accept = {};
            if (attrs.hasOwnProperty('disabled')) {
                var disabled = (attrs.disabled === 'true');
                if (disabled) accept.disabled = disabled;
            }
            var text = $('<input />', {
        ...
        createImage: function(attrs) {
            var accept = {};
            var image = $('<div />', {
                'class': 'motopress-property-image'
            });

            var tools = $('<div />', {
                'class': 'motopress-image-tools'
            });
            $('<div />', {
            ...
            if (attrs.hasOwnProperty('disabled')) {
                ...
                if (disabled) {
                    image.append($('<div />', {
                        'class': 'motopress-image-disabled'
                    }));
                }
            }
            ...
            $.each(attrs.list, function(name, value) {
                ...
                select.append(
                    $('<option />', {
                        value: name,
                        text: value
                    }).attr('data-content', '<span class="color ' + classPrefix + colorClass +
        ...
        createSlider: function(attrs, name) {
            var slider = $('<div>', {
                'class': 'motopress-property-slider'
            });
            var span = $('<span>', {
                type: 'text',
        ...
        createRadioButtons: function(attrs) {
            ...
            var group = $('<div />', {
                'class': 'motopress-property-radio-buttons'
            });
        ...
        createGroup: function(attrs) {
            var accept = {};
            var wrapper = $('<div />', {
                'class': 'motopress-property-group'
            });
        ...
        createMargin: function(props) {
            var table = $('<table />', {
                'class': 'motopress-property-margin'
            });
            $.each(props.sides, function(i, side) {
                var tr = $('<tr />', {
                    'data-motopress-margin-side': side
                }).appendTo(table);
                $('<td />').append(
                    $('<i />', {
                        'class': props.classPrefix + 'icon ' + props.classPrefix + 'icon-' + side
                    })
                ).appendTo(tr);
                $('<td />').append(
                    $('<div />', {
            ...
            var datepickerWrapper = $('<div />', {
                'class': 'motopress-property-datetime-picker'
            });
	    var divForm = $('<div />', {
		'class' : 'form-group'
	    });
	    var divWrapper = $('<div />', {
		'class' : 'input-group date motopress-property-datetime-picker-wrapper '
	    });
            attrs.readonly = 'readonly';
	    var input = this.createInput(attrs, name, undefined, 'datetime-picker-input form-control');

	                var buttonSelector = $('<span />' ,{
		'class': 'input-group-addon motopress-property-datetime-picker-button'
	    }); 
	    var iconButton = $('<span />' ,{
		'class': 'motopress-calendar-icon fa fa-calendar'
	    });
        ...
        createStyleEditor: function(attrs, name){
            var accept = {};
            var styleEditorWrapper = $('<div />', {
                'class': 'motopress-property-style-editor-wrapper' 
            });
        ...
        createComplexCtrl: function(attrs, name){
            var accept = {};
            var wrapper = $('<div />', {
                'class': 'motopress-property-complex'
            });            
            var innerForm = $('<div />', {
                'class': 'motopress-property-inner-form'
            });
        ...
        renderShortcode: function(status) {
            if (!this.isGrid) {
                ...
                var handle = this.element
                    .closest('.motopress-clmn')
                    .find('.' + CE.DragDrop.myThis.helper.attr('class'))
                    .first()
                    .children('.' + CE.DragDrop.myThis.dragHandle.attr('class'));
                ...
                if ( (status === 'created') && parameters) parameters = this.settingsCtrl.setDefaultAttrs(parameters);

                if ( (status === 'created') && styles) styles = this.stylesCtrl.setDefaultAttrs(styles);
                ...
                $.ajax({
                    ...
                    success: function(dirtyData) {
                        ...
                        var parameters = CE.LeftBar.myThis.getParametersAttrs($this.group, $this.shortcodeName),
                            childParams, groupControl;

                        if ($this.groupItemName) {
                            groupControl = $this.settingsForms.find('.motopress-property-group').controller();
                            ...
                            if ( (status === 'created') ) {
                                if (childParams.hasOwnProperty('items') && childParams.items.hasOwnProperty('count') && childParams.items.count > 0) {
                                    $this.settingsForms.find(
                                        '> [data-motopress-parameter="' + $this.groupItemName + '"] > .motopress-property-group > .motopress-property-button-wrapper > .motopress-property-button-default'
                                    ).trigger('click', childParams.items.count);
                                }

                                if (groupControl.dependency) {
                                    $this.settingsForms
                                        .find('[data-motopress-parameter="' + groupControl.dependency.parameter + '"] .motopress-controls')
            ...
            $.each(properties, function(name, props) {
                ...
                switch (props.type) {
                    ...
                    case 'longtext-tinymce':
                        ...
                        form.ce_ctrl_textarea_tinymce($.extend(controlSettings, {
                            currentShortcode: $this.formsMainCtrl.element
                        }));
                        wrapper.append(legend, label, form, description);
                        break;
                    ...
                    case 'media':
                        var mediaReturnMode = (typeof props.returnMode === 'undefined' || $.inArray(props.returnMode, ['id', 'url']) === -1) ? 'url' : props.returnMode;
                    ...
                    case 'multi-images':
                        ...
                        form.ce_ctrl_image_slider($.extend(controlSettings, {
                            autoOpen: props.autoOpen
                        }));
                        wrapper.append(legend, label, form, description);
                        break;
                    ...
                    case 'group':
                        var shortcodeCtrl = $this.formsMainCtrl.shortcode.controller(),
                            childName = props.contains;
                        shortcodeCtrl.setGroupItemName(name);
                        shortcodeCtrl.setChildName(childName);
                        if (props.hasOwnProperty('activeParameter') && props.activeParameter) shortcodeCtrl.setActiveParameter(props.activeParameter);
                    ...
                    case 'radio-buttons' :
                        ...
                        form.ce_ctrl_radio_buttons($.extend(controlSettings, {
                            value: props.value
                        }));
                    ...
                    case 'icon-picker' :
                        ...
                        form.ce_ctrl_icon_picker($.extend(controlSettings, {
                            value: props.value
                        }));
                    ...
                    case 'datetime-picker' : 
                        ...
                        var displayMode = (typeof props.displayMode === 'undefined' || $.inArray(props.displayMode, ['date', 'datetime']) === -1) ? 'datetime' : props.displayMode;
                    ...
                    case 'margin':
                        ...
                        form.ce_ctrl_margin($.extend(controlSettings, {
                            margin: props
                        }));
                        wrapper.append(legend, label, form, description);
                        break;
...
steal('jquery/controller', function($) {
    CE.ControlsForm('CE.StyleEditorControlsForm', {}, {
        ...
        generateStatesWrapper: function(){
            var $this = this;
            this.statesWrapper = $('<div />', {
                'class': 'motopress-ce-style-editor-states-wrapper'
            });
            this.statesWrapper.append(
                $('<a />', {
                    'href': '#',
                    'data-state': 'up',
        ...
        generatePropertiesWrapper: function(){
            this.propertiesWrapper = $('<div />', {
                'class': 'motopress-ce-style-editor-properties-wrapper'
            });
        ...
        generateActionsWrapper: function(){
            var $this = this;
            this.actionsWrapper = $('<div />', {
                'class': 'motopress-ce-style-editor-actions-wrapper'
            });            
            var applyBtn = $('<button />', {
            ...
            var dropdownBtnWrapper = $('<div />', {
                'class': 'btn-group motopress-bootstrap-dropdown motopress-dropdown-button dropup'
            }).append(applyBtn, '<button class="dropdown-toggle btn" data-toggle="dropdown"><span class="caret"></span></button>', actionsMenu);

            this.actionsWrapper.append(dropdownBtnWrapper);
            this.element.append(this.actionsWrapper);

	        dropdownBtnWrapper.on('keydown', function(e) { e.stopPropagation(); });
        },
        ...
        getEmptyValues: function(){
            var generalAttrs = this.getGeneralAttrs();
            var $this = this;
            return $.map(generalAttrs, function(){return $this.emptyStyleValue;});
        },
        ...
        generateActionButtons: function(){
            ...
            var btns = [
                $('<a />', {
					...
					,'title': localStorage.getItem('CELiteTooltipText')
                })
        ...
        getStateAttrs: function(state){
            ...
            switch (state) {
                case 'up':
                    ...
                    break;
                case 'hover':
                    $.extend(attrs, this.getPresetStateAttrs('up'), this.getPresetStateAttrs('hover'),
                        this.getPrivateStateAttrs('up'), this.getPrivateStateAttrs('hover'));
                    break;
                case 'tablet':
                    $.extend(attrs, this.getPresetStateAttrs('up'), this.getPresetStateAttrs('tablet'),
                        this.getPrivateStateAttrs('up'), this.getPrivateStateAttrs('tablet'));
                    break;
                case 'mobile':
                    $.extend(attrs, this.getPresetStateAttrs('up'), this.getPresetStateAttrs('tablet'),
                        this.getPresetStateAttrs('mobile'), this.getPrivateStateAttrs('up'),
                        this.getPrivateStateAttrs('tablet'), this.getPrivateStateAttrs('mobile'));
                    break;
            }
            return attrs;
        },                
...
steal('jquery/controller', function($) {
    CE.ControlsForm('CE.StyleControlsForm', {}, {
                    ...
                    switch (styleAttrs[name].type) {
                        case 'margin':
                            ...
                            $.each(styleAttrs[name]['default'], function(i, val) {
                                if (val !== styleAttrs[name].values[0]) allNone = false;
                            });
        ...
        getTargetElement: function(ctrl){
            ...
            if ( ( ctrl.name === 'mp_style_classes' || ctrl.name === 'mp_custom_style' )
                && CE.LeftBar.myThis.library[this.shortcodeGroup].objects[this.shortcodeName].styles[ctrl.name].hasOwnProperty('selector')
            ){
        ...
        _save: function(shortcodeStyle, ctrl, change) {
            ...
            switch (ctrl.name) {
                case 'margin':
                    ...
                    if (oldMarginClasses.length) oldValue = oldMarginClasses;

                    var marginClasses = ctrl.getClasses(value);
                    if (marginClasses.length) value = marginClasses;
            ...
            if (selectorEl !== null && selectorEl.length) {
                if (change) selectorEl.removeClass(oldValue);
...
steal('jquery/controller', function($) {
    CE.ControlsForm('CE.SettingsControlsForm', {}, {
                    ...
                    if (this.shortcodeName === 'mp_row' || this.shortcodeName === 'mp_row_inner') {
                        switch(ctrl.name) {
                            ...
                            case 'stretch':
                                switch(value) {
                                    ...
                                    case 'full':
                                        ...
                                        this.setRowWidth(contentWidth === '' ? 'mp-row-fullwidth' : 'mp-row-fullwidth-content');
                                        break;
                                    case 'fixed':
                                        this.setRowWidth('mp-row-fixed-width');
                                        break;
                                }
                            ...
                            case 'width_content':
                                switch(value){
                                    case '':
                                        this.setRowWidth('mp-row-fullwidth');
                                        break;
                                    case 'full':
                                        this.setRowWidth('mp-row-fullwidth-content');
                                        break;
                                }
                                parent.MP.Editor.triggerIfr('Resize');
                                break;
            ...
            if (shortcodeAttrs) {
                ...
                $.each(generalAttrs, function(name, props) {
                    ...
                    if (el.hasClass('motopress-property-group')) {
                        if (change) el.trigger('change', isNew);
                        el.find('.motopress-property-group-accordion-item').each(function(){
                            var childFormCtrl = $(this).controller();
        ...
        clearRowWidth: function(){
            var allWidthClasses = 'mp-row-fullwidth mp-row-fullwidth-content mp-row-fixed-width';
        ...
        clearRowMediaBG: function(element){
            ...
            element
                .removeClass('mp-row-video motopress-row-parallax')
    ...
    CE.SettingsControlsForm('CE.SettingsControlsChildForm', {}, {
            ...
            if (shortcodeAttrs) {
                ...
                $.each(generalAttrs, function(name, props) {
                    ...
                    if (
                        name === 'id'
                        && $this.shortcodeName === 'mp_accordion_item'
        ...
        init: function() {
            ...
            $(document).on('keydown', '[data-motopress-shortcode="mp_text"], [data-motopress-shortcode="mp_heading"]', function(e) {
                if (e.which === $.ui.keyCode.TAB) {
                    ...
                    if (CE.InlineEditor.curElement) CE.InlineEditor.curElement.controller().close(true, 'show');
                    var handle = $(this)
                        .closest('.motopress-clmn')                                                
                        .find('.' + CE.DragDrop.myThis.helper.attr('class'))
                        .first()
            ...
            $(parent.document).on('mousedown', function(e) {
                if (CE.InlineEditor.curElement && !$(e.target).closest('.ui-dialog').length) CE.InlineEditor.curElement.controller().close(true, 'show');
            });
        },
        ...
        isTinymce: function(e) {
            ...
            if (!$(e.target).hasClass('motopress-drag-handle')) {
                ...
                if (
                    clickedEl.length
                    && (
                        clickedEl.closest('[data-motopress-shortcode]').length
                        || clickedEl.closest('.mce-tinymce').length
                        || clickedEl.closest('.mce-window').length
                        || clickedEl.closest('.mce-popover').length
                        )
                    )
                {
                    isTinymce = true;
...
steal('jquery/controller', function($) {
    ...
    {},
    {
        ...
        saveHandler: function(e) {
            var $this = parent.CE.CodeModal.myThis;
            $this.switchVisual();
            var controller = parent.CE.CodeModal.currentShortcode.controller();
...
steal( 'jquery/class', function($) {
    ...
    },
    {
        ...
        removeBlock: function() {
            ...
            if (CE.Dialog.myThis.element.dialog('isOpen')) CE.Dialog.myThis.element.dialog('close');
            ...
            if (spanRemovable.hasClass('ce_inline_editor')) {
                var ctrl = spanRemovable.controller();
            ...
            if (!isUnwrapped) CE.DragDrop.myThis.resize(rowFrom, spanRemovable, 'remove', newSizes);
    ...
    },
    {
        ...
        makeResizable: function(obj) {
            ...
            obj.not('[data-motopress-wrapper-id], .motopress-empty').each(function() {
				var shortcode = $(this).find('.motopress-block-content > [data-motopress-shortcode]'),
					shortcodeName = shortcode.length ? shortcode.attr('data-motopress-shortcode') : null,
					handleClass;
        ...
        horizontalResize: function(handleClass, ui) {
            ...
            if (newSpanWidth === this.oldWidth) return false;
            ...
            for (var i = 1; i <= maxCols; i++) {
                ...
                if (newSpanWidth >= snapGird[i] - half / 2 && newSpanWidth <= snapGird[i] + half) {
                    ...
                    if (empty && empty.length) {
                        ...
                        if (diff < originalEmptyNumber) CE.Resizer.myThis.changeSpanClass(empty, parent.CE.Iframe.myThis.gridObj.span.class + (originalEmptyNumber - diff));
                        else empty.remove();
                        ...
                        if (direction2 === 'east') ui.element.after(emptySpanClone);
                        else ui.element.before(emptySpanClone);
                    }
        ...
        verticalResize: function(ui) {
            ...
            if(minHeight === newSpanHeight) ui.element.css('min-height', '');
        ...
        hideSplitters: function(exclude) {
            ...
            if (!exclude) splitters.addClass('motopress-hide');
            else splitters.not(exclude).addClass('motopress-hide');
        },
...
steal('jquery/class', function($) {
    ...
    },
    {
        ...
        emptySceneHelper: $('<section />', {
            id: 'motopress-empty-scene-helper'
        }),

        emptySceneDroppable: $('<div />', {
            'class': 'motopress-empty-scene',
            html:
                '<div class="motopress-empty-scene-container-first"><div class="motopress-empty-scene-image"></div><div class="motopress-empty-scene-wrapper-right"><div class="motopress-empty-scene-title">' + localStorage.getItem('CEEmptySceneHelperTitle') + '</div><div class="motopress-empty-scene-content">' + localStorage.getItem('CEEmptySceneHelperContent') + '</div></div></div>'
                +
                '<div class="motopress-empty-scene-container-second"><div class="motopress-empty-scene-or-label">'+localStorage.getItem('CEEmptySceneHelperOr')+'</div><input type="text" class="motopress-empty-scene-type-here" value="'+localStorage.getItem('CEEmptySceneHelperInputText')+'"></div>'
        }),

        templates: $('<div />', {
            'class': 'motopress-templates-container',
            html:
                '<div class="motopress-templates-title">' +
                    '<div class="motopress-templates-separator"><div class="motopress-templates-separator-line"></div></div>' +
                    '<div class="motopress-templates-label">'+localStorage.getItem('CEChooseLayout')+'</div>' +
                    '<div class="motopress-templates-separator"><div class="motopress-templates-separator-line"></div></div>' +
                '</div>' +
                '<div id="motopress-templates"></div>'
        }),
        ...
        wrapperHelper: $('<section />', {
            'class': 'motopress-wrapper-helper'
        }),

        wrapperHelperResizer: $('<section />', {
            'class': 'motopress-helper'
        }),
        ...
        init: function() {
            ...
            this.overlay.append(
                this.handleIntermediate.clone(),
                this.handleLeft.clone(),
                this.handleTopIn, this.handleBottomIn,
                this.handleLeftOut, this.handleRightOut,
                this.handleLeftIn, this.handleRightIn,
                this.handleRight.clone()
            );
            this.helper.append(this.overlay, this.focusArea.clone(), this.dragHandle.clone(),  this.resizer.splitter.clone());

            this.emptySpanOverlay.append(
                this.handleIntermediate.clone(),
                this.handleLeft.clone(),
                this.handleRight.clone(),
	            this.handleInsert.clone()
            );
            this.emptySpanHelper.append(this.emptySpanOverlay, this.resizer.splitter.clone());

            this.wrapperHelper.append(
                this.handleIntermediate.clone(),
                this.handleLeft.clone(),
                this.handleRight.clone()
            );
            ...
            this.emptySceneHelper
                .append(this.emptySceneDroppable, this.templates)
                .appendTo($('.motopress-content-wrapper'));

            this.helperContainer.append(
                this.lineHelperIntermediate, this.lineHelperLeftOut, this.lineHelperRightOut, this.lineHelperLeft, this.lineHelperRight, this.lineHelperLeftIn, this.lineHelperRightIn,
                this.lineHelperTopIn, this.lineHelperBottomIn, this.lineHelperHandleMiddle, this.lineHelperInsert,
                this.textHelperIntermediate, this.textHelperLeftOut, this.textHelperRightOut, this.textHelperLeft, this.textHelperRight, this.textHelperLeftIn, this.textHelperRightIn,
                this.textHelperTopIn, this.textHelperBottomIn, this.textHelperHandleMiddle, this.textHelperInsert
            );
        ...
        setEdges: function(){
            var editableRows = $('.motopress-content-wrapper .motopress-row')
                                    .not('.motopress-content-wrapper [data-motopress-group]:not([data-motopress-group="mp_grid"]) .motopress-row'); 
            CE.DragDrop.setEdgeRow(editableRows);

                        var editableSpans = $('.motopress-content-wrapper .motopress-clmn')
                                    .not('.motopress-content-wrapper [data-motopress-group]:not([data-motopress-group="mp_grid"]) .motopress-clmn'); 
        ...
        main: function() {
	        if (!this.inited) this.sceneEvents();
            var editableRows = $('.motopress-content-wrapper .motopress-row')
                                    .not('.motopress-content-wrapper [data-motopress-group]:not([data-motopress-group="mp_grid"]) .motopress-row'); 
            var editableSpans = $('.motopress-content-wrapper .motopress-clmn')
                                    .not('.motopress-content-wrapper [data-motopress-group]:not([data-motopress-group="mp_grid"]) .motopress-clmn'); 
            if (!this.inited) this.addEmptySceneHelper();
            if (!this.inited) this.addHandleMoreDisable();
	        ...
	        if (this.isEmptyScene()) CE.Utils.addSceneState('empty-scene');
        ...
        makeEmptySceneHelperFocusable: function() {
	        $('.motopress-empty-scene-type-here').off('focusin').on('focusin', function() {
                CE.DragDrop.myThis.emptySceneHelper.hide().css('opacity', 1);
                var rowTo = $(this).closest('.motopress-row');
                var rowFrom = null;
                var block = CE.DragDrop.myThis.newBlock.clone();
                ...
                var rowTo = CE.DragDrop.myThis.rowHtml.clone().find('.motopress-filler-content').replaceWith(block).end();
                CE.DragDrop.myThis.makeRowEditable(rowTo, true);
                $('.motopress-content-wrapper').append(rowTo);

                var t1 = setTimeout(function() {
                    CE.DragDrop.myThis.addHandleMiddle(rowTo, parent.CE.Iframe.myThis.gridObj.span.fullclass);
                    clearTimeout(t1);
                }, 0);
        ...
        makeRowEditable: function(obj, isNew) {
            ...
            obj.each(function() {
                ...
                if (!$rowEdge.children('.motopress-row-helper').length) {
                    var rowHelper = $('<span />', {
                        'class' : 'motopress-row-helper'
                    });
                    var rowSelectHandle  = $('<div />', {
                        'class' : 'motopress-row-select-handle'
                    });
                    rowHelper.append(rowSelectHandle.clone());
                    $rowEdge.append(rowHelper.clone());
                    var shortcode = $this.attr('data-motopress-shortcode');
                    if (isNew || !shortcode) CE.DragDrop.myThis.setAttrs($this);
                }
                if (typeof $this.controller(CE.Controls) === 'undefined') $this.ce_controls({isNew: isNew});
                var ctrl = $this.controller(CE.Controls);
        ...
        makeEditable: function(obj, ui, isNew) {
            ...
            obj.each(function() {
                ...
                if (!$this.closest('.motopress-block-content').length) {
                    ...
                    CE.DragDrop.myThis.makeDroppable();
                    if (typeof ui !== 'undefined' && ui !== null) CE.DragDrop.myThis.setAttrs($this, ui);

                    if (isNew) $this.addClass('motopress-new-object');
                    $this.ce_controls({isNew: isNew});
                    var spanCtrl = $this.controller(CE.Controls);
                    if (isNew && spanCtrl) {
                    ...
                    if (blockContent.length) {
                        ...
                        var ctrl = shortcode.controller(CE.Controls);
                        if (ctrl) (isNew) ? ctrl.renderShortcode(isNew) : ctrl.render(false);
        ...
        getAttrsFromElement: function(sourceEl){
            ...
            if (typeof parameters !== 'undefined') attrs.parameters = parameters;
            var styles = sourceEl.attr('data-motopress-styles');
            if (typeof styles !== 'undefined') attrs.styles = styles;
            return attrs;
        },
        ...
        _setAttrs: function(el, source) {
            if (typeof source === 'undefined') {
                ...
                if (source.hasOwnProperty('parameters')) el.attr('data-motopress-parameters', source.parameters);
                if (source.hasOwnProperty('styles')) el.attr('data-motopress-styles', source.styles);
            }
        },
        ...
        makeDraggableNewBlock: function() {
            var $this = this,
	            ...
	            contentWrapper = $('.motopress-content-wrapper:first'),
	            contentWrapperTop = contentWrapper.length ? contentWrapper.offset().top : 0;
            ...
            $('.motopress-ce-object').draggable({
                ...
                start: function(e, ui) {
                    ...
                    if (selectedDragHandle.length) CE.Selectable.myThis.unselect(selectedDragHandle);
            ...
            this.emptySceneDroppable.droppable({
                ...
                drop: function(e, ui) {                    
                    ...
                    var t = setTimeout(function() {
                        ...
                        if (flag) spanClass = 'motopress-content-wrapper';
        ...
        makeDroppable: function() {
            ...
            $('.motopress-content-wrapper').find('.motopress-handle-intermediate, .motopress-handle-left, .motopress-handle-top-in, .motopress-handle-bottom-in, .motopress-handle-left-out, .motopress-handle-right-out, .motopress-handle-left-in, .motopress-handle-right-in, .motopress-handle-middle-in, .motopress-handle-insert, .motopress-handle-right').droppable({
                ...
                drop: function(e, ui) {
	                ...
	                if (
		                typeof $this.hoveredHandles !== 'undefined'
		                && $this.hoveredHandles.length > 1
		                && $this.hoveredHandles.filter('.motopress-handle-left, .motopress-handle-right').length
	                ) {
						if ($.inArray(handle, ['left', 'right']) > -1) {
							$this.canDrop = true;
						} else {
							return false;
						}
                    ...
                    spanClassDraggable = CE.DragDrop.myThis.removeEmptyBlocks(block, spanClassDraggable);
                        ...
                        case 'top-in':                            
                            ...
                            if ((newSizes = CE.DragDrop.myThis.canBeMoved(rowFrom, null, fromPos, toPos, myUI)) !== false) {
                                ...
                                wrapperSpan.attr({
                                    'data-motopress-wrapper-id': CE.DragDrop.myThis.wrapperId
                                }).prepend(CE.DragDrop.myThis.wrapperHelperResizer.clone());
                        ...
                        case 'left-out': case 'left': case 'intermediate':                                    
                        ...
                        case 'insert':
                            ...
                            if (span.parent().length) span.replaceWith(block);
        ...
        showLineTextHelper: function(span, handle) {
	        var spanOffset = span.offset(),
                ...
                linePiece, lineWidth, lineLeft, textLeft, width,
	            lineBorder = 1, lineBorders = lineBorder * 2,
	            nextSpan, lineHelper, textHelper;
	        ...
	        switch(handle) {
                ...
                case 'middle-in':
                    ...
                    if (span[0] === handleMiddleFirst[0] ||
                        span[0] === $('.motopress-content-wrapper > .motopress-handle-middle-in:last')[0]
                    ) {
                        var contentWrapper = $('.motopress-content-wrapper');
        ...
        canBeMoved: function(rowFrom, rowTo, fromPos, toPos, ui, handle, interactingSpan, silent) {
            ...
            if (rowTo !== null) {
                ...
                if (handle && handle === 'out' && rowFromEdge !== null && rowFromEdge[0] === rowToEdge[0]) return true;
            ...
            if (spanCount < spanLimit || rowToEdge === null) {
                ...
                var realIndex, sizes, item, hasNotAllowed, needToShare, canToShare, shared, accepted, enableSkipIndex,
                    fromNewSizes = new Array(), toNewSizes = new Array(),
                    rowFromWidth, rowToWidth,
                    sameContainerWrapperIndex = null, sameContainerWrapperWidth = null,
                    from = new Array(), to = new Array(),
                    loopCounter = 0;
                ...
                if (rowFrom !== null) {
                    ...
                    if (rowFromSpanCount >= 1) {
                        ...
                        rowFromChildren.each(function (index) {
                            ...
                            if (!hasNotAllowed && !item.isAllowed) hasNotAllowed = true;
                            from.push(item);
                        });
                ...
                if (allowSpanWidth) {
                    ...
                    if (handle && handle !== 'out' && handle !== 'insert') {
                        ...
                        rowToChildren.each(function (index) {
                            ...
                            if (index === toPos) {
                                to.push(newItem);
                                if (!hasNotAllowed && !newItem.isAllowed) hasNotAllowed = true;
                                realIndex++;
                            }
	    ...
	    sceneEvents: function() {
		    this.container.off('hover').hover(
		        function() {
...
steal('jquery/controller', function($) {
    ...
    },
    {
        template: $('<div />', {
            ...
            html: '<img class="motopress-template-icon">' +
                '<p class="motopress-template-name"></p>'
        }),
...
steal('jquery/class', function($) {
    ...
    },
    {
        ...
        init: function() {
            ...
            $(document).mousedown(function() { CE.Selectable.setScrollY(window.scrollY); });
            $(document).mouseup(function() { CE.Selectable.setScrollY(window.scrollY); });
            this.makeSelectable();
        },
        ...
        makeSelectable: function() {
            $('.motopress-content-wrapper').on('click', '.motopress-drag-handle, .motopress-clmn-select-handle, .motopress-row-select-handle', function() {
                ...
                if (
                    $(this).hasClass('.motopress-clmn-select-handle') && $(this).closest('.motopress-clmn').hasClass(CE.Selectable.myThis.selectedClass)
                    || $(this).hasClass('.motopress-row-select-handle') && $(this).closest('.motopress-row').hasClass(CE.Selectable.myThis.selectedClass)
                    || $(this).hasClass(CE.Selectable.myThis.selectedClass)
                ) {
                    ...
                    if ($(this).hasClass('.motopress-row-select-handle')) {
                        ...
                        selectedElement = $(this)
                            .closest('.motopress-clmn')                            
                            .find('.motopress-block-content').first()
                                .children('[data-motopress-shortcode]');
                    }

                    var ctrl = selectedElement.controller();
                    var inlineEditorIsOpen = (selectedElement.hasClass('ce_inline_editor')) ? ctrl.isOpen : false;
                    if (
                        (selectedElement.hasClass('ce_controls') || selectedElement.hasClass('ce_inline_editor') || selectedElement.hasClass('ce_code_editor') || selectedElement.hasClass('.motopress-row')) &&
                        !CE.Dialog.myThis.element.dialog('isOpen') &&
                        !parent.CE.CodeModal.myThis.element.data('modal').isShown &&
                        !inlineEditorIsOpen
                    ) {
            ...
            $(document).on('click', function(e) {
                if (
                    !$(e.target).hasClass(CE.Selectable.myThis.selectedClass)
                    && !$(e.target).hasClass('motopress-clmn-select-handle')
                    && !$(e.target).hasClass('motopress-row-select-handle')
                    && !$(e.target).closest('.' + CE.Dialog.myThis.dialogClass).length
                    && !CE.InlineEditor.isTinymce(e)
                ) {
                    CE.Selectable.myThis.unselect();
                }
            });
        ...
        select: function(el) {
            if (CE.DragDrop.interruptInsert === false) {
                ...
                if(el.hasClass('motopress-row-select-handle')){
                    ...
                    .closest('.motopress-helper')
                        .nextAll('.ui-resizable-handle:not(".motopress-hide")')
                            .show();
                    container = el.closest('.motopress-clmn');
                }
                ...
                if (
                    (container.hasClass('ce_controls') || container.hasClass('ce_inline_editor') || container.hasClass('ce_code_editor') || container.hasClass('motopress-row')) &&
                    !CE.Dialog.myThis.element.dialog('isOpen')
                ) {
        ...
        unselect: function() {
            ...
            if (selected.length) {
                ...
                if (CE.Dialog.myThis.element.dialog('isOpen')) CE.Dialog.myThis.element.dialog('close');
                if (CE.InlineEditor.curElement) CE.InlineEditor.curElement.controller().close();
...
steal('jquery/class', function($) {
    ...
    },
    {
        ...
        getPresetsListSelect2: function(){
            var list = [];
            $.each(this.presetStyles, function(index, presetObj){
                list.push({id: presetObj.getClassName(), text: presetObj.getLabel()});
            });
            return list;
        },
...
steal('jquery/class', function($) {
    ...
    $.Class('CE.ShortcodeStyle',
    {
        ...
        generateCSS: function(className, settings, emulateMode){
            if (typeof emulateMode === undefined) emulateMode = false;
